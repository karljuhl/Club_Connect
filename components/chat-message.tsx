...
export function ChatMessage({ message, chatbotLogoURL }: ChatMessageProps) {
    return (
            ...
            <div className="flex-1 px-1 ml-4 space-y-2 overflow-hidden">
                {message.content === "loading" ? (
                    <>
                        ...
                        {/* Optionally, display the chatbot logo even during loading if available */}
                        {chatbotLogoURL ? (
                            <Image src={chatbotLogoURL} alt="Assistant Logo" width={32} height={32} className="rounded-full" />
                        ) : (
                            <Icons.bell />
                    ...
                    <MemoizedReactMarkdown
                        ...
                        components={{
                            a({ node, children, ...props }) {
                                return <ExternalLink href={props.href}>{children}</ExternalLink>;
                            },
                            p({ children }) {
                            ...
                            code({ node, className, children, ...props }) {
                                ...
                                if (match && (match[1] === 'math' || match[1] === 'latex')) {
                                    return <MathJaxContext><MathJax>{children}</MathJax></MathJaxContext>;
                                }
                                return <CodeBlock key={Math.random()} language={match && match[1]} value={String(children).replace(/\n$/, '')} {...props} />;
                            }
                        }}
                    >{message.content.replace(/\【.*?】/g, "")}</MemoizedReactMarkdown>
                )}
            </div>
        </div>
    );
